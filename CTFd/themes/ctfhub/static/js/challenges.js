...
function marksolves(cb) {
  $.get(script_root + "/api/v1/" + user_mode + "/me/solves", function(
    ...
    for (var i = solves.length - 1; i >= 0; i--) {
      ...
      btn.prepend("<i class='fa fa-check corner-button-check'></i>");
    }
    if (cb) {
      cb();
    }
  ...
  $.get(script_root + "/api/v1/challenges", function(response) {
    ...
    for (var i = 0; i <= challenges.length - 1; i++) {
      ...
      if (user_solves.indexOf(chalinfo.id) == -1) {
        ...
        var chalbutton = $(
          "<button class='btn btn-dark challenge-button solved-challenge w-100 text-truncate pt-3 pb-3 mb-2' value='{0}'><i class='fa fa-check corner-button-check'></i></button>".format(
            chalinfo.id
          )
        );
      }
